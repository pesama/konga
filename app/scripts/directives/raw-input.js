...
angular.module('sigmaNgApp')
  .directive('rawInput', function() {
	    return {
	      ...
	      controller: function($scope, $filter, $modal, $timeout, common, api, $rootScope, configurationManager, standardApi, permissionManager) {
			...
			/**
	  		...
	  		$scope.validation = {
	  			pattern: function() {
	  				...
	  				if ($scope.property.validation.validators && $scope.property.validation.validators.length > 0
	  					...
	  					var regexp = new RegExp($scope.property.validation.validators[0].value);
	  					return regexp;
	  				}
	  		...
	  		// TODO Change this!
	  		$scope.classFormInput = ($scope.property.fieldType[$scope.mode] != constants.FIELD_PICK_LIST) ? "form-input-content" : "";
	  		$scope.displayMode = ($scope.property.fieldType[$scope.mode] != constants.FIELD_PICK_LIST) ? "" : "pickListDispBlock padding-cero";
	      ...
	      link: function(scope, element, attrs) {
			...
			scope.contentUrl = '/views/raw-' + fieldType.toLowerCase() + inputSuffix + '-input.html';
